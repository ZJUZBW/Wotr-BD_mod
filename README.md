...
- ~~临时分享的 ACTestingGrounds 的完整汉化文件 [ACTestingGrounds.zip](https://github.com/nixgnot/WOTR-CONTENTS-MODS-CN/files/12715542/ACTestingGrounds.zip)。配合[MOD外挂汉化工具QuickLocalization](https://github.com/nixgnot/WOTR-CONTENTS-MODS-CN#mod外挂汉化工具quicklocalization)使用~~【已经整合进QL汉化了】。
...
拿长剑，转无甲以后搭配 圣战连击（长剑）+神圣武器技法（爱奥梅黛）【主角是诡计大师则拿坟中歌者巨斧会更好】，禁忌学识可以扩表拿~~法师法术护盾术~~【搞错了】等，配合圣武士4环死亡化身和次元斩击，属于是能抗能打，辅助能力一流。
...
> - 目前存在一个BUG（原版就有），在计算巫术等级的时候，冬巫的等级不会跟萨满叠加（冬巫只跟巫师等级叠加计算巫术DC），而是只计算萨满的等级。要用好邪眼，就需要保证萨满8级，所以只能放弃邪眼这类8级强化的巫术了。
...
狩魔猎手/圣职杀手是审判官完全体（偷袭骰+研究目标+战斗自然秘示域+扩表金鹰之魂和神使灵光，配合新法术死亡化身、次元斩击和神灵化身，前期就很强，8级开始就是超级战神），人类开局可以发挥最大强度，兰恩用这个也是无敌。

**加点**：19力、18敏、12体、10智、18感、5魅；这里多了购点是混种人用洗点MOD的BUG，让兰恩可以额外选择一次种族加值，我们把他放感知属性上。

**神话加点**：1第二秘示域（自然）-马、2精通重击（神话）、3常备不怠、4
...
1法术专攻（塑能）、机敏庇护主、渡鸦魔宠
2【巫术】：沉眠
3法术专精（燃烧之手-燃烧之弧-火球术，一路换下去）、元素专攻（寒冷）
5超魔（法术增效）
6【巫术】：邪眼
7瓦瑞西安刺青（塑能）
9超魔（法术强效）
...
神话加点：1偏好超魔（增效），2能量转换、3偏好超魔（强效）、4元素链接（寒冷）
...
神话加点：1替换为神话专长-神话专攻（咒法），2反应施术、3、4扩充战法（惑控）
```
...
- A类附伤：机敏之刃、法打（冻伤）、纵火狂之戒、马兰德的耻辱腰带、圣触（空间斩/~~次元斩击~~ 打接触攻击）、元素狂潮（被冰封禁制超魔以后一半转火焰触发；感觉不是很赚，TTT新增能量转换以后还是用能量转换吧）
- B类附伤：神话冲锋、破敌、神圣公正附魔、诡诈烈焰手套
- 配合道途附伤，一刀500+轻轻松松。可惜这套玩法不能用飞刃袭之类的法术战斗也不能用次元斩击了，施法会导致冻伤的持续效果消失（类似专注一样）。这套玩双手魔战士一样可以玩，只要学个元素超魔（火焰）就行。
...
**神话加点**：1变化自如 2能量转换 3巅峰元素（火） 4神话冲锋/充裕施法 5
...
- 要动物伙伴，则靠专长点出动物伙伴，配合【精通法术共享】【注：这个团队专长需要动物伙伴和索希尔一起点出来，但是动物伙伴身上的精通法术共享不要激活，只激活索希尔的；否则动物伙伴分享到的BUFF时间会虚空减半---因为会再平分给一个并不存在的角色】，养一个强力动物伙伴（说实话比起强力菜刀，动物伙伴的输出再怎么搞也是菜得抠脚，输出占比在5%左右，最多也就10%；要是你队伍各个位置都已经有成型队友，就不需要额外弄这种资源利用效率极低的动物伙伴了）。
...
- 不要动物伙伴（或者就拿个花豹当T），主塑能系法术（观星者星座能力之一可以给火焰描述符的法术+2施法者等级），配合TTT的新增神话-元素链接和能量转换，玩转火焰的颂圣之语可以一路秒杀和麻痹。
...
看了桌面规则里的地狱骑士执法官的开卡，发现只有1级地狱骑士也满足RP，相当于就在地狱骑士挂了个职。

**加点**：13力 17敏 16体 10智 10感 16魅，升级全加敏捷，背景不变，种族替换选旅者

**专长加点**：

```
---通用加点参考-------
1突变斗士：异种武器擅长（穿甲剑）、双武器战斗
2突变斗士：双斩
3突变斗士：武器专攻（穿甲剑）
4突变斗士：包抄
5突变斗士：进阶武器训练：轻巧双武、【武器训练：重刃】
6地狱骑士：【神爪骑士团】
7原怒者：【天界血脉】、精通双武器战斗
8鳞甲之拳：鹤形拳
9受诅巫师：【跛足】、【冻皮】、精通重击（穿甲剑）
10原怒者：
11原怒者：
12原怒者：
13原怒者：
...
神话加点：1无限狂暴、2武器娴熟（神话）、3屹立不倒/待定、4
